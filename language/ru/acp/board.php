...
$lang = array_merge($lang, array(
	...
	'ALLOW_CDN_EXPLAIN'				=> 'Если включено, некоторые служебные файлы конференции будут загружаться с внешних серверов вместо собственного сервера. Это уменьшит потребность вашего сервера в траффике, но для некоторых администраторов может стать проблемой с точки зрения сохранения конфиденциальности.',
	...
	'LOAD_JQUERY_CDN'				=> 'Загружать jQuery из библиотек Google CDN',
	'LOAD_JQUERY_CDN_EXPLAIN'		=> 'Если включено, jQuery будет загружен из библиотек Google AJAX API CDN вместо использования копии из пакета phpBB на сервере конференции. Если CDN недоступен, phpBB попытается использовать собственную копию библиотеки.',
...
$lang = array_merge($lang, array(
	...
	'AUTH_PROVIDER_OAUTH_EXPLAIN'				=> 'Каждый провайдер OAuth требует уникальную пару секретного кода и ключа для аутентификации с помощью внешнего сервера.<br />Они должны быть предоставлены провайдером и ведены в точности в том виде, в котором были получены.<br />Любой сервис, не предоставляющий одновременно и ключ, и секретный код, будет недоступен для аутентификации пользователей конференции.',
...
$lang = array_merge($lang, array(
	...
	'MOD_REWRITE_ENABLE_EXPLAIN' => 'Если включено, адреса URL, содержащие ’app.php’, будут изменены так, чтобы в них не значилось имя исполняемого файла (например: app.php/foo будет выглядеть как /foo). <strong>Для работы данной функции требуется включить модуль mod_rewrite сервера Apache; если она включена в отсутствие модуля mod_rewrite, адреса URL на конференции могут отображаться неверно.</strong>',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_PASSWORD_RESET_EXPLAIN'	=> 'Определяет, могут ли пользователи конференции использовать ссылку «Я забыл свой пароль» на странице входа для восстановления доступа к своему аккаунту. Если используется внешний механизм аутентификации, может понадобиться отключить данную функцию.',
