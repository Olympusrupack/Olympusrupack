...
$lang = array_merge($lang, array(
	...
	'MOD_REWRITE_ENABLE_EXPLAIN' => 'Если включено, адреса URL, содержащие ’app.php’, будут изменены так, чтобы в них не значилось имя исполняемого файла (например: app.php/foo будет выглядеть как /foo). <strong>Для работы данной функции требуется перенаправление всех несоответствующих реальным файлам или директориям адресов на app.php/ с помощью вебсервера или его расширения,</strong> к примеру модуля mod_rewrite сервера Apache; если она включена в отсутствие этого условия, адреса работать не будут.',
